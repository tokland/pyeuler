...
def problem38():
    ...
    def concatenated_product(number, max_digits):
        ...
        return first(ds for ds in products if len(ds) >= max_digits)
    # 987654321 is the maximum pandigital, so 9876 seems a reasonable upper bound 
    return max(ifilter(is_pandigits(imap(concatenated_product, xrange(2, 9876+1))))
